...
import 'package:prophecy_compagnon_shared/builders/session_map_items_generator.dart';
import 'package:source_gen/source_gen.dart';

Builder registerSessionMapItemsBuilder(BuilderOptions options) {
  var defaults = BuilderOptions({
    'output': 'lib/classes/session/map/register_map_items.dart'
  });

  var opts = options.overrideWith(defaults);

  return RegisterSessionMapItemsBuilder(options: opts);
}

class RegisterSessionMapItemsBuilder implements Builder {
  RegisterSessionMapItemsBuilder({ required this.options })
      : generator = RegisterSessionMapItemsGenerator();

  final BuilderOptions options;
  final RegisterSessionMapItemsGenerator generator;
