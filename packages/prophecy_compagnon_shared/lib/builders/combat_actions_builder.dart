...
import 'package:prophecy_compagnon_shared/builders/combat_actions_generator.dart';
import 'package:source_gen/source_gen.dart';

Builder registerCombatActionsBuilder(BuilderOptions options) {
  var defaults = BuilderOptions({
    'output': 'lib/classes/session/encounter/register_combat_actions.dart'
  });

  var opts = options.overrideWith(defaults);

  return RegisterCombatActionsBuilder(options: opts);
}

class RegisterCombatActionsBuilder implements Builder {
  RegisterCombatActionsBuilder({ required this.options })
    : generator = RegisterCombatActionsGenerator();

  final BuilderOptions options;
  final RegisterCombatActionsGenerator generator;
