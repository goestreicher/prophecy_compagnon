/*
 * Copyright (C) 2026 Grégory Oestreicher
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */

import 'package:build/build.dart';
import 'package:source_gen/source_gen.dart';

class RegisterCombatActionsGenerator extends Generator {
  @override
  String generate(LibraryReader library, BuildStep buildStep) {
    var lines = <String>[];

    for(var element in library.classes) {
      if(!element.isPublic) continue;
      if(element.isAbstract) continue;

      var isCombatAction = false;
      for(var interfaceType in element.allSupertypes) {
        if(interfaceType.getDisplayString() == 'CombatAction') isCombatAction = true;
      }
      if(!isCombatAction) continue;

      lines.add(
        '  CombatAction.registerCombatActionJsonFactory(\n'
        '    "${element.name}",\n'
        '    (Map<String, dynamic> json) => ${element.name}.fromJson(json),\n'
        '  );\n'
      );
    }

    return lines.join('\n');
  }
}